...
uint8_t XBOXUSB::Init(uint8_t parent, uint8_t port, bool lowspeed) {
        ...
        if(VID != XBOX_VID && VID != MADCATZ_VID && VID != JOYTECH_VID && VID != GAMESTOP_VID && VID != EASYSMX_1_VID && VID != EASYSMX_2_VID && VID != XBOX_DUPLICATE_1_VID && VID != XBOX_DUPLICATE_2_VID) // Check VID
...
#endif
                goto FailUnknownDevice;
        } else if(PID != XBOX_WIRED_PID && PID != MADCATZ_WIRED_PID && PID != GAMESTOP_WIRED_PID && PID != AFTERGLOW_WIRED_PID && PID != JOYTECH_WIRED_PID && PID != EASYSMX_WIRELESS_1_PID && PID != EASYSMX_WIRELESS_2_PID && PID != XBOX_DUPLICATE_WIRELESS_1_PID && PID != XBOX_DUPLICATE_WIRELESS_2_PID) // Check PID
                goto FailUnknownDevice;
